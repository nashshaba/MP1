...
### Step 3: Calculate RMSLE for each model 
```{r}
# add predicted values to pretend test set to compare predicted values and actual outcome and calculate RMSLE
...
# create a df for 5 RMSLEs from 5 folds of pretend training and test sets, and calculate their mean as  predicted RMSLE
rmsle <- rbind(e1, e2, e3, e4, e5) %>% 
  mutate(model_number = c(1, 2, 3, 4, 5)) %>% 
  select(model_number, rmsle)
rmsle
predicted_rmsle <- mean(rmsle$rmsle)
predicted_rmsle
```
...
rmsle_hat <- c(0.3187564,0.315039,0.3149522,0.314817,0.3147375,0.3147397,0.3148387,0.315039,0.3327549)
new_df <- data.frame(df,rmsle_hat)
as_tibble(new_df)
plot_df <- ggplot(new_df,aes(x=df,y=rmsle_hat))+
  geom_point()
plot_df
...
op_d<-ggplot(dat2,aes(x=df,y=rmlse_hat))+
  geom_point()+
  scale_y_continuous(limits=c(0.313, 0.335))+
  scale_x_continuous(limits=c(0,35),breaks=seq(0,35,5),expand=c(0,0))+
   geom_vline(xintercept=19,linetype = "dashed", colour = "red")+
  annotate("segment", x=19,xend=22, y=0.325, yend= 0.325)+
  annotate("text", x= 24, y=0.325, label= "df* = 19", size=5,  fontface="italic")
  # expand_limits(y=c(0.313, 0.335))
op_d
>>>>>>> aea729b7e0138a03637ba90e378a78a882c9df38
```
***
# Submission 
...
This section relates to the **reaching for the stars** step of the grading scheme: showing that your estimated $\widehat{\text{rmsle}}$ you obtained from your implemented crossvalidation scheme is "close" to the real $\text{rmsle}$ that Kaggle returns on the leaderboard.

Our estimated $\widehat{\text{rmsle}} = .3147375$ with$ df\star = 19$. The real $\text{rmsle} = .32798$ from Kaggle.  The two values are very close. 